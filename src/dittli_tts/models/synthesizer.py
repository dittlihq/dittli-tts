...
class VariationalDurationModel(nn.Module):
    def __init__(
        ...
        for i in range(n_flows):
            self.flows.append(modules.ConvolutionalFlow(2, filter_channels, kernel_size, n_layers=3))
            self.flows.append(modules.FlipTransform())

        self.post_pre = nn.Conv1d(1, filter_channels, 1)
        self.post_proj = nn.Conv1d(filter_channels, filter_channels, 1)
        self.post_convs = modules.DepthwiseSepConv(filter_channels, kernel_size, n_layers=3, p_dropout=p_dropout)
        self.post_flows = nn.ModuleList()
        self.post_flows.append(modules.AffineCoupling(2))
        for i in range(4):
            self.post_flows.append(modules.ConvolutionalFlow(2, filter_channels, kernel_size, n_layers=3))
            self.post_flows.append(modules.FlipTransform())

        self.pre = nn.Conv1d(in_channels, filter_channels, 1)
        self.proj = nn.Conv1d(filter_channels, filter_channels, 1)
        self.convs = modules.DepthwiseSepConv(filter_channels, kernel_size, n_layers=3, p_dropout=p_dropout)
        if gin_channels != 0:
...
class PhonemeEncoder(nn.Module):
    ...
    def forward(self, x, x_lengths, tone, language, bert, ja_bert, g=None):
        ...
        x_mask = torch.unsqueeze(commons.create_length_mask(x_lengths, x.size(2)), 1).to(x.dtype)
...
class VoiceSynthesizer(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.sdp = VariationalDurationModel(hidden_channels, 192, 3, 0.5, 4, gin_channels=gin_channels)
        self.dp = DurationEstimator(hidden_channels, 256, 3, 0.5, gin_channels=gin_channels)
    ...
    def forward(
        ...
        x_enc, m_p, logs_p, x_mask = self.enc_p(x, x_lengths, tone, language, bert, ja_bert, g=g_p)
        ...
        with torch.no_grad():
            ...
            if self.use_noise_scaled_mas:
                eps = torch.std(neg_cent) * torch.randn_like(neg_cent) * self.current_mas_noise_scale
                neg_cent = neg_cent + eps

            attn_mask = (torch.unsqueeze(x_mask, 2) * torch.unsqueeze(y_mask, -1)).squeeze(1)  # [b, t_y, t_x]
        ...
        l_length_dp = torch.sum((logw_pred - logw_target) ** 2 * x_mask) / torch.sum(x_mask)
    ...
    def infer(
        ...
        y_mask = torch.unsqueeze(commons.create_length_mask(y_lengths, None), 1).to(x_mask.dtype)
